...
public class PlayerService {
    ...
    public void subHPPlayer(Player player, double hp) {
        if (!player.isDie()) {
            player.nPoint.subMP((long) hp);
    ...
    public void sendTNSM(Player player, byte type, double param) {
        if (param > 0) {
            ...
            try {
                msg = new Message(-3);
                msg.writer().writeByte(type);// 0 là cộng sm, 1 cộng tn, 2 là cộng cả 2
                msg.writer().writeDouble(Dragon.utils.Util.limitDouble(param));// số tn cần cộng
                player.sendMessage(msg);
                msg.cleanup();
            } catch (Exception e) {
    ...
    public void sendInfoMp(Player player) {
        ...
        try {
            ...
            msg.writeDouble(Dragon.utils.Util.limitDouble(player.nPoint.mp));
            player.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Logger.logException(PlayerService.class, e);
    ...
    public void sendInfoHpMpMoney(Player player) {
        ...
        try {
            ...
            msg.writeDouble(Dragon.utils.Util.limitDouble(player.nPoint.hp));// chp
            msg.writeDouble(Dragon.utils.Util.limitDouble(player.nPoint.mp));// cmp
                    ...
                    if (!player.isBoss && !player.isPet && !player.isTrieuhoipet && !player.isClone) {
                        if (x < 24 || x > player.zone.map.mapWidth - 24 || y < 0
                            ...
                            if (MapService.gI().getWaypointPlayerIn(player) == null) {
                                ChangeMapService.gI().changeMap(player, 2, 0, 528, 360);
                                return;
                            }
                        }
    ...
    private static final int COST_GOLD_HOI_SINH = 5000;
