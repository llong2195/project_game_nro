package Dragon.models.npc.specialnpc;
...
public class MagicTree {
    ...
    public void openMenuTree() {
        ...
        try {
            ...
            if (!isUpgrade) {
                ...
                msg.writer().writeUTF("Nâng Nhanh\n9\nNgọc");
    ...
    public void upgradeMagicTree() {
        // Vàng nâng cấp x2 mỗi level
        int goldRequire = 5000 * (int) Math.pow(2, this.level - 1);
    ...
    public void unupgradeMagicTree() {
        // Trả lại 50% vàng đã nâng cấp
        int goldRequire = 5000 * (int) Math.pow(2, this.level - 2); // Vàng của level trước
        int goldReturn = goldRequire / 2; // Hoàn lại 50%
        this.player.inventory.gold += goldReturn;
        PlayerService.gI().sendInfoHpMpMoney(this.player);
        this.isUpgrade = false;
        if (this.level > 1) {
            this.level--; // Giảm cấp độ
        }
        this.loadMagicTree();
    }

    public void fastRespawnPea() {
        // Kết hạt nhanh: reset số lượng hạt
        this.currPeas = this.getMaxPea();
        this.loadMagicTree();
    }

    public void fastUpgradeMagicTree() {
        // Nâng cấp nhanh: tăng cấp mà không cần vàng
        if (this.level < MAX_LEVEL) {
            this.level++;
        }
        this.isUpgrade = false;
